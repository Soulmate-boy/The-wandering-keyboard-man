...
(function (factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD. Register as anonymous module.注册为匿名模块。
  ...
  var $window = $(window),
    ...
    EVENT_RESIZE = 'resize' + CROPPER_NAMESPACE, // Bind to window with namespace使用名称空间绑定到窗口
  ...
  // Custom proxy to avoid jQuery's guid定制代理以避免jQuery的guid
  ...
  prototype.load = function (url) {
    ...
    $clone.one('load', $.proxy(function () {
      ...
      /* #134: Seems like the order is important for Chrome. 似乎这个顺序对Chrome很重要。*/
      crossOrigin: crossOrigin,
      src: bustCacheUrl || url
    });

    // Hide and insert into the document隐藏并插入文档
  ...
  prototype.build = function () {
    ...
    // Create cropper elementsCreate cropper elements
    this.$cropper = $cropper = $(Cropper.TEMPLATE);

    // Hide the original image24/5000  	  // 隐藏原始图像
    $this.addClass(CLASS_HIDDEN);

    // Show the clone iamge展示克隆的iamge
  ...
  prototype.unbuild = function () {
    ...
    this.container = null;
    this.canvas = null;
    this.cropBox = null; // This is necessary when replace这在更换时是必要的
  ...
  $.extend(prototype, {
    ...
    renderCanvas: function (changed) {
      ...
      if (this.rotated) {
        ...
        // Computes rotatation sizes with image sizes使用图像大小计算旋转大小
        rotated = getRotatedSizes({
          width: image.width,
          height: image.height,
          degree: image.rotate
    ...
    initCropBox: function () {
      ...
      // Initialize auto crop area初始化自动作物面积
    ...
    limitCropBox: function (size, position) {
      ...
      if (size) {
        ...
        if (aspectRatio) {
          // compare crop box size with container first首先用容器修剪作物箱的大小
        ...
        // The "minWidth" must be less than "maxWidth", and the "minHeight" too.69/5000  
		  // “minWidth”必须小于“maxWidth”，“minHeight”也必须小于“maxWidth”。
